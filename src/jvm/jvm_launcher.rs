...
#[cfg(feature = "dev")]
use {
    crate::args_parser::LaunchTarget, crate::util::jvm_util::parse_classpath, std::env, std::iter,
};

const JAVA_CLASS_PATH_VM_ARG_PREFIX: &str = "-Djava.class.path=";

pub fn jvm_launch(launcher_arg: &LauncherArg) -> Result<(), MessageError> {
    let vm_ops = launcher_arg.vm_args();

    let mut args_builder = jni::InitArgsBuilder::new().version(JNIVersion::V8);
    ...
    match launcher_arg.classpath() {
        Some(classpath) => {
            let cp = match &target {
                LaunchTarget::Class(_) => env::join_paths(classpath),
                Jar(jar) => env::join_paths(classpath.iter().chain(iter::once(jar.path()))),
            }
        ...
        None => match &target {
            LaunchTarget::Class(_) => {
                if let Ok(cp) = env::var("CLASSPATH") {
                    let cp = env::join_paths(parse_classpath(&cp))
                        .expect("failed to handle class path!");
    ...
    let result = run(launcher_arg, &jvm, env_ref);
    destroy_vm(&jvm, env_ref);
    result
}

fn run(
    launcher_arg: &LauncherArg,
    jvm: &jni::JavaVM,
    env_ref: &mut JNIEnv,
) -> Result<(), MessageError> {
    ...
    let target = launcher_arg.target();
    let main_class = jni_result_expect!(
        env_ref,
        helper.check_and_load_main(env_ref, target),
        &format!("can not load main class: {}", target.main_class())
    )?;
...
fn call_main(
    ...
    let args = jni_result_expect!(
        env,
        env.new_object_array(
            jsize::from(app_args.len() as i32),
            "java/lang/String",
            JObject::null()
        )
    )?;
    ...
    for (i, item) in app_args.iter().enumerate() {
        jni_result_expect!(
            ...
            env.set_object_array_element(
                &args,
                jsize::from(i as i32),
                jni_result_expect!(env, env.new_string(item))?
            )
        )?;
    }
    ...
    jni_result_expect!(
        env,
        env.call_static_method(main_class, "main", "([Ljava/lang/String;)V", &params)
    )?;
