...
impl<'local> JvmLauncherHelper<'local> for SunLauncherHelper<'local> {
    fn check_and_load_main(
        ...
        let name = env
            .new_string(target.target_value())
            .unwrap_or_else(|_| panic!("path convert failed: {}", target.target_value()));
...
impl SimpleLauncherHelper<'_> {
    pub fn from_env<'a>(env: &mut JNIEnv<'a>) -> jni::errors::Result<SimpleLauncherHelper<'a>> {
        ...
        let class_loader_object = env.call_static_method(
            &class_loader_class,
            GET_SYSTEM_CLASS_LOADER_METHOD,
            GET_SYSTEM_CLASS_LOADER_METHOD_DESC,
            &[],
        )?;
...
impl<'local> JvmLauncherHelper<'local> for SimpleLauncherHelper<'local> {
    fn check_and_load_main(
        ...
        let name = env
            ...
            .unwrap_or_else(|_| panic!("path convert failed: {class_name}"));
...
pub fn find_launcher_helper_from_env<'a>(
    env: &mut JNIEnv<'a>,
) -> Result<LauncherHelper<'a>, MessageError> {
    ...
    Ok(LauncherHelper::SimpleLauncherHelper(
        SimpleLauncherHelper::from_env(env)
            .map_err(|e| MessageError::new(&format!("cannot init launcher helper: {e}")))?,
    ))
}
